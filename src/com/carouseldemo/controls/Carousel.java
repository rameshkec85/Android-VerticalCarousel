...
public class Carousel extends CarouselSpinner implements
	...
	public Carousel(Context context, AttributeSet attrs, int defStyle) {
		...
		mAnimationDuration = arr.getInteger(
				R.styleable.Carousel_android_animationDuration, 400);
		...
		int min = arr
				.getInteger(R.styleable.Carousel_minQuantity, MIN_QUANTITY);
		int max = arr
				.getInteger(R.styleable.Carousel_maxQuantity, MAX_QUANTITY);
...
//	}
	...
	protected boolean getChildStaticTransformation(View child,
		...
		Log.v("Carousel:"+((CarouselItem) child).getIndex(), "X: "+((CarouselItem) child).getX()+" Y:"+((CarouselItem) child).getY()+" Z: "+((CarouselItem) child).getZ());
		...
		mCamera.translate(((CarouselItem) child).getX(),
				((CarouselItem) child).getY(), ((CarouselItem) child).getZ());
		...
		float rot = ((CarouselItem) child).getZ();

		rot = (rot / 360);
		
		matrix.preTranslate(-centerX, -centerY);
		//matrix.postScale(rot, rot);
		matrix.postTranslate(centerX, centerY);
		//matrix.setScale(rot, rot);
		...
		mm.setValues(values);
		((CarouselItem) child).setMatrix(mm);
...
try{
	...
	void trackMotionScroll(float deltaAngle) {
		...
		for (int i = 0; i < getAdapter().getCount(); i++) {
			...
			Calculate3DPosition(child, getHeight(), angle);
		}
